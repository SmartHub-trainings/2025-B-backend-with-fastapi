...
import os 
from sqlalchemy import create_engine,Column,Integer,String,DateTime,Date,ForeignKey,select,Float
...
def get_event_by_id(id:int,db:Session =Depends(get_db)):
    query = select(Event).where(Event.event_id==id)
    event=db.execute(query).scalar_one()
    print(event)
    return {
        "message": "Events retrieved successfully",
        "success":True,
...
@app.get("/events/{event_id}/tickets",tags=["Events"])
def get_ticket_by_event_id(event_id:int,db:Session =Depends(get_db)):
    try:
        ...
        print(e)
        return {
            "message": "Tickets retrieved failed",
            "success":False,
            "statusCode":400
        }
...
def create_ticket_handler(body:TicketCreate,db:Session =Depends(get_db)):
    try:

        query = select(Event).where(Event.event_id==body.event_id)
        event = db.execute(query).scalar_one()
        if not event:
            return {
                "message": "Event not found",
        ...
        print(e)
        return {
            "message": "Tickets created failed",
            "success":False,
            "statusCode":400
        }

@app.post("/bookings",tags=["Bookings"])
def create_booking_handler(body:BookingCreate,db:Session =Depends(get_db)):
    try:
        query = select(Ticket).where(Ticket.ticket_id==body.ticket_id)
        ticket = db.execute(query).scalar_one()
        if not ticket:
            return {
        ...
        print(error)
        return {
            "message": "Bookings created failed",
            "success":False,
            "statusCode":400
        }   
